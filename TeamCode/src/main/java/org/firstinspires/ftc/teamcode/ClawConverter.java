...
public class ClawConverter //This will F up in jGrasp, needs to be executed in Android Studio
{
  ...
  */
    ...
    private final double servoDeadMin = 500.0; //I have these set at pwm values 500,2500
    private final double servoDeadMax = 2500.0;
    private final double pwmToRadPhase = -500.0; //so the values are read 0-2000 [applied before scalar]
    private final double pwmToRadScal = (Math.PI)/1000.0;
    ...
    private void checkForceWrist () //desired Pos should probably be set in pwm val (500-2500)
    {
        ...
        if((forcePositivity && (distFrom > -5))||(!(forcePositivity) && (distFrom < 5)))
        {
            forceSetWrist = -1;
        }
    }
    ...
    private void ultiPrimeMove(double thetaP, double bP, double wMan)//theta prime, belt prime, wrist manual
    {
        ...
        if(forceSetWrist < 0)
        {
            ...
            wrist.setPosition(idealWristPwm);//MethodDNE
        }
    /*else
    {
       wrist.setPosition(forceSetWrist);//Is it an issue if I repeatedly call set position to the same spot? //MethodDNE
        ...
        //elbow
        double idealElbowPwm = elbow.getPosition() + invSrvRad(thetaP);//MethodDNE
        elbow.setPosition(idealElbowPwm);//MethodDNE
        //belt
  ...
  */
    ...
    public void servoStagger(double stagLen, /*double t,*/ double stiHoriz, double stiVert, double trigInput)//stagLen = length of time for each deriv in milisec, t = time
    {
    ...
    //ACTUAL PUBLIC------------------V
    //SAME THING NO TIMER
    public void noTimerStagger(double stagLen, /*double t,*/ double stiHoriz, double stiVert, double trigInput)//stagLen = length of time for each deriv in milisec, t = time
    {
        //obv distance/time = speed, time = stagLen, distance = (getPos+=amnt), speed = desired prime
